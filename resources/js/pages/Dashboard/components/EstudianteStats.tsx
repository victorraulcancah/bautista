                    ...
                    {cursos.map((c: any) => (
                        <div key={c.id} className="p-6 rounded-[2rem] border border-gray-100 hover:border-indigo-100 bg-white hover:shadow-xl hover:shadow-indigo-50/50 transition-all flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                                <div className="size-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-lg group-hover:scale-110 transition-transform">
                                    {c.curso?.nombre?.[0] || 'C'}
                                </div>
                                <div>
                                    <h4 className="font-black text-gray-900 leading-none">{c.curso?.nombre ?? '—'}</h4>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-1">{c.apertura?.nombre ?? ''}</p>
                                </div>
                            </div>
                            <Link 
                                href={`/alumno/cursos/${c.id}`}
