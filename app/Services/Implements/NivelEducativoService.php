<?php

namespace App\Services\Implements;

use App\Models\NivelEducativo;
use App\Repositories\Interfaces\NivelEducativoRepositoryInterface;
use App\Services\Interfaces\NivelEducativoServiceInterface;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Pagination\LengthAwarePaginator;

class NivelEducativoService implements NivelEducativoServiceInterface
{
    public function __construct(
        private readonly NivelEducativoRepositoryInterface $repository,
    ) {}

    public function listar(int $instiId, string $search = '', int $perPage = 15): LengthAwarePaginator
    {
        return $this->repository->paginate($instiId, $search, $perPage);
    }

    public function todos(int $instiId): Collection
    {
        return $this->repository->all($instiId);
    }

    public function obtener(int $id): NivelEducativo
    {
        return $this->repository->findById($id);
    }

    public function crear(array $data): NivelEducativo
    {
        return $this->repository->create($data);
    }

    public function actualizar(int $id, array $data): NivelEducativo
    {
        $nivel = $this->repository->findById($id);
        return $this->repository->update($nivel, $data);
    }

    public function eliminar(int $id): void
    {
        $nivel = $this->repository->findById($id);
        $this->repository->delete($nivel);
    }
}
